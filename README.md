...
- `TropicalGT-I/configs/train_full_dataset_pg_bpb_step0_full24b_b55_v11_bpb_5k_gate.json` is the latest committed strict full-dataset Parameter-Golf BPB template; the live b60 step-0 5K gate run uses a generated launch config under `TropicalGT-I/outputs/launch_configs/`. `TropicalGT-I/configs/train_full_dataset_active.json` is an older full-dataset path, and `TropicalGT-I/configs/train.json` is a legacy cap-sized review config.
...
On this workstation that path may be a user-created symlink to an ignored checkout, but committed full-dataset configs no longer contain `fallback_roots` or `tokenizer_fallback_paths`. Required data and tokenizer paths must resolve directly, with `allow_config_path_fallbacks: false`, so missing paths fail instead of silently substituting another source. Populate the full SP1024 cache before a full run:
...
The certificate metrics are finite graph-structure checks: edge tokens are rewarded when their active tropical support lies on the edge itself or one of its endpoint vertex tokens. The raw `certificate_loss` is the negative log mass that the tropical-support distribution assigns to these allowed support sets. If it rises while margins grow, the model is confidently routing to supports outside the current structural certificate, usually visible through `certificate_disallowed_support_rate`, `certificate_allowed_mass_mean`, and the node/edge/graph support-rate breakdowns. These certificates are useful for auditing whether tropical supports follow the TokenGT graph skeleton, but they are not semantic correctness proofs and must be interpreted beside task loss, verifier scores, and held-out BPB.
...
Recent no-proxy visualization contracts are machine-validated by `TropicalGT-I/scripts/validate_interactive_audit_artifacts.py`: `tropicalgt.local_embedding_neighborhood_surface.v1` for observed-anchor local NLL sheets, `tropicalgt.analogical_map_layout.v1` for probability-vector correspondence pages, backend provenance on `tropicalgt.persistence_landscape.v1`, `tropicalgt.embedding_trajectory_identity.v1` for graph-state PCA trajectory identity, and `tropicalgt.simplex_tree_readability.v1` for SimplexTree summary-first pages. Dense SimplexTree cover graphs are not the default view; pages show GUDHI provenance, dimension counts, filtration histograms, representative inclusions, and make the full actual face-to-coface cover trace legend-only when it exceeds the readability threshold.

Training checkpoints contain model state, optimizer state, current step, metrics, history, config, and RNG state. Checkpoint writes are atomic: the trainer writes a temporary file, verifies it is nonempty and loadable by default, fsyncs, and then replaces the target path. Set `checkpoint_verify_load: false` only for explicitly constrained runs; the nonempty atomic-write guard still remains active. Resume a run by pointing `train_tropicalgt_i.py` at a final or `.latest.pt` checkpoint:
...
Analogical memories are inserted through a quality gate, not stored blindly. Configure `memory_quality_*` thresholds to require model-probability Jensen-Shannon complexes, topological algebra payloads, nonnegative NLL improvement, and any additional score/BPB/margin thresholds. Early training may legitimately retrieve no memories until trajectories clear that gate; sample audit pages still render the model output, but rejected samples do not enter the memory bank.
...
When reviewing from a local Codex browser, forward the remote port rather than copying artifacts:

```bash
ssh -o BatchMode=yes -o ExitOnForwardFailure=yes -fN -L 127.0.0.1:8991:127.0.0.1:8991 iska-tailscale
```

Open `http://127.0.0.1:8991/`. These `outputs/` files, symlinks, PID files, logs, and browser bundles are generated artifacts and must not be staged.
...
--config TropicalGT-I/configs/train_full_dataset_pg_bpb_step0_full24b_b55_v11_bpb_5k_gate.json \
--checkpoint TropicalGT-I/checkpoints/tropicalgt_i_pg_bpb_step0_full24b_b55_v11_bpb_5k_gate.latest.pt \
