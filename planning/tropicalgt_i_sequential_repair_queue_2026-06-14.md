...
- [x] Found legacy dataset path-search fields (`fallback_roots`, `tokenizer_fallback_paths`) in full-dataset configs even though token-id fallback was disabled; implemented strict path handling so those fields fail closed under the no-proxy/no-fallback policy with no opt-in resolver path.
