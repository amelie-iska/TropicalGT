...
The sidecar now also carries `tropicalgt.vector_bundle_paper_sidecar_completeness.v1`. This separates `unavailable`, `telemetry_partial`, and `paper_ready` evidence tiers. `paper_ready` is allowed only when chart/transport ids, configured toric active rows, flat-incidence diagnostics, GraphCG-toric agreement metrics, and transported persistence-landscape metrics are all present from actual run data. The strict interactive-artifact validator rejects any sidecar that claims paper-ready vector-bundle evidence while its completeness contract still lists missing required groups. This completeness tier remains separate from theorem, toric/tropical embedding, normal-fan, scheme, sheaf, and CAS certificates.
