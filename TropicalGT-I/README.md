...
Run `TropicalGT-I/scripts/audit_metric_provenance.py` to refresh `TropicalGT-I/outputs/metric_provenance_audit.{json,md}`. The registry labels exact BPB, graph-conditioned BPB, GraphCG condition diagnostics, NLL plot surrogates/interpolants, persistence vectorizations, algebraic free-resolution proxies, graph JSON fallbacks, and stripped-export estimates. Uncovered generic mentions are intentionally left visible in the report until each is either registered, renamed, or removed.
