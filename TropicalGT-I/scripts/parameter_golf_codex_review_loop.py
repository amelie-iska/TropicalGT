...
def _review_prompt(
    *,
    ...
    return (
        ...
        "You are the main Codex coordinator for a graph-encoded OpenAI Parameter-Golf BPB run.\n"
        f"The run reached review boundary step `{boundary_step}`. The target is `{metric} < {target_bpb}`.\n"
        "First, spawn or assign a fresh Codex subagent for the evidence review when subagent tools are available; "
        "the subagent must inspect all metrics, advanced sidecars, validation reports, W&B summaries, and the "
        "topological, geometric, algebraic, GraphCG, GFlowNet, memory, and decoding visualizations listed in the "
        "artifact inventory.\n"
...
def _active_training_contract(cfg: dict[str, Any], report: dict[str, Any], checkpoint: dict[str, Any], boundary_step: int, report_path: Path | None = None, target_bpb: float | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "active_losses": {
            ...
            "tropical_margin_signed_loss": metrics.get("tropical_margin_signed_loss", metrics.get("tropical_margin_loss")),
            "tropical_margin_reward": metrics.get("tropical_margin_reward"),
            "tropical_margin_shortfall_loss": metrics.get("tropical_margin_shortfall_loss"),
            "tropical_margin_shortfall_rate": metrics.get("tropical_margin_shortfall_rate"),
