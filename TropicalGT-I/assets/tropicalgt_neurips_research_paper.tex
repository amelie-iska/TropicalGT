...
\]
TropicalGT additionally reports graph-aware scores.  Let \(B\) be the number of predicted target bytes, \(S(G)\) the deterministic structural byte count of the actually tokenized graph tokens, and \(J(G)\) the explicit JSON byte count of non-derived graph side information after removing the deterministic text path.  With side-information weight \(\rho\), the implementation logs
\[
