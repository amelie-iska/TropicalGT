...
TropicalGT-I uses persistence-landscape vectors as optional geometric evidence for retrieval.  Two complementary tropical vector bundle theories give a more structural way to decide how such evidence should be transported, compared, and regularized across graph charts.  The first is the tropical toric theory of Khan and Maclagan, where a toric tropical vector bundle is encoded by a valuated matroid plus compatible ray filtrations.  The second is the semiring-scheme theory of Jun, Mincheva, and Tolliver, where vector bundles are locally free modules over tropical schemes and transition functions are forced, under zero-sum-free hypotheses, to be monomial matrices.  For TropicalGT-I this suggests a practical design rule: local graph charts may carry different tropical coordinates, but feature transports between charts should be sparse monomial rescalings together with matroidal filtration constraints rather than arbitrary dense affine maps.

\paragraph{Reference-theory background.}
Khan and Maclagan's tropical toric vector bundles are not merely collections of piecewise-linear functions.  Their basic datum is a simple valuated matroid $M$ on a ground set $G_M$ together with decreasing flat filtrations $E^i(j)$ along the rays of a fan; the vector-bundle condition requires every maximal cone to admit a matroid basis whose ray-threshold joins recover those flats.  Their Cox-semirings and bend-congruence presentation explain why fibers are tropical linear spaces and why realizability depends on both the valuated matroid and the chosen filtrations.  Jun, Mincheva, and Tolliver give the complementary semiring-scheme warning: over zero-sum-free local semirings with only trivial idempotent pairs, invertible changes of basis are monomial and many vector bundles split as line-bundle sums.  TropicalGT-I uses the first result as a matroidal compatibility template and the second as a constraint on invertible chart transports, while treating expressive neural chart covers as a metric-gated training construction rather than as literal tropical schemes.
...
\]
where $M$ is a valuated matroid on a ground set of feature atoms, $p$ is its circuit valuation, $\mathcal{F}^{a,i}(j)$ is a decreasing filtration of flats indexed by the tropical ray or graph statistic $i$ in chart $a$, and each overlap transport is a tropical monomial map
\[
  T_{ab}(z)_k = c_{ab,k}+z_{\pi_{ab}(k)}.
\]
The bundle is chart-consistent on $G$ if $T_{bc}T_{ab}=T_{ac}$ on triple overlaps up to a tropical scalar and if every active filtration can be recovered as a join of atoms selected by tropical ray inequalities.
\end{definition}

The definition mirrors the toric compatibility condition for tropical toric vector bundles: over each maximal cone, the ray filtrations must be generated by a matroid basis.  In TropicalGT-I the cones are not literal fans of a toric variety; they are local regions in activation space defined by graph neighborhoods, persistence summaries, and token context.  The toric condition is nevertheless useful because it gives an auditable finite constraint: for each chart, selected basis atoms must reconstruct the flats that determine which features are allowed to dominate.
...
This definition turns toric geometry into a trainable interface.  The exponent rows are small integer templates for edge cuts, motif counts, persistence-landscape bins, register states, and token-window statistics.  Their max-linear images produce a toric tropical coordinate system in which local feature comparisons are cheap and in which chart changes can be checked by sparse permutations and shifts.

\begin{definition}[Ray-filtration atomization for graph tokens]
\label{def:ray-filtration-atomization}
Let $P_i$ be graph-token probes such as endpoint incidence, edge type, active tropical support, GraphCG direction activity, persistence-bin membership, or memory-retrieval availability.  A ray-filtration atomization maps each probe to a ray score $s_i(u)$ for feature atom $u\in G_M$ and defines
\[
...
\begin{proposition}[Finite auditability of ray-filtration losses]
\label{prop:finite-ray-auditability}
...
\begin{proof}
All sets in Definition~\ref{def:ray-filtration-atomization} are finite on a batch: graph tokens are finite, feature atoms are finite, and the threshold grid is finite.  Matroid closure and rank-defect penalties are therefore table operations on subsets of $G_M$.  The toric active-cell terms depend on finitely many max-linear probe rows, and the GraphCG term depends on finitely many learned directions.  Persistence and memory terms enter only through stored finite summaries or availability masks.  Thus the audit evaluates a finite list of ranks, closures, active rows, and distances.
\end{proof}
...
\]
for all active rays $i$ and thresholds $j$, then the chart bundle satisfies the toric tropical vector-bundle compatibility condition on the fan cells represented by the charts.  A nonnegative relaxation is
\[
...
\paragraph{Training principle: GraphCG directions as toric rays.}
GraphCG learns latent steering directions.  A tropical toric embedding can treat a small subset of stable, full-rank GraphCG directions as candidate ray probes $v_i$ in the filtration terms $\mathcal{F}^{a,i}(j)$.  The corresponding regularizer penalizes disagreement between the active GraphCG direction cone and the active toric normal-fan cell:
\[
