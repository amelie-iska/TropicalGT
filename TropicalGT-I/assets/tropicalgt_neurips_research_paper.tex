...
\newenvironment{objectivecard}[7]{%
  \par\smallskip\noindent\begingroup
  \setlength{\fboxsep}{6pt}%
  \fcolorbox{darkblue}{softblue}{%
    \begin{minipage}{0.94\linewidth}
    \textbf{Objective #1: #2}\par
    \textit{Task.} #3\par
    \textit{Theory.} #4\par
    \textit{Acceptance.} #5\par
    \textit{Metrics.} #6\par
    \textit{Guardrail.} #7
    \end{minipage}}%
...
\subsection{Finite tropical-to-toric sidecars}
\label{subsec:finite-tropical-toric-sidecars}

The active-face dictionary can be made algebraic without claiming that a Transformer layer is itself a toric variety.  TropicalGT-I uses a local finite sidecar: a checkpoint, layer, head, token window, and rationalized probe determine a Laurent-polynomial or toric-ideal object that can be audited by toric and tropical methods.  This sidecar is a certificate for the sampled tropical computation, not a global model of the whole network.

Let $N\simeq \mathbb Z^r$ be a cocharacter lattice, $M=\operatorname{Hom}(N,\mathbb Z)$, and $T_N=\operatorname{Spec}K[M]$ over a nonarchimedean field $K$ with uniformizer $\tau$ and $\operatorname{val}(\tau)=1$.  A rationalized TropicalGT head or low-rank audit probe supplies characters $a_1,\ldots,a_R\in M$ and candidate scores
\[
  \ell_r(u)=\langle a_r,u\rangle+b_r,\qquad u=P h\in N_\mathbb R,\quad b_r\in\mathbb Q.
\]
Clearing denominators records an integer lattice object together with a scale factor.  Define
\[
  f=\sum_{r=1}^R \tau^{-b_r}\chi^{a_r}\in K[M].
\]
TropicalGT uses the max-plus convention, while many computer-algebra packages use the nonarchimedean min convention.  The bridge is stored explicitly:
\[
  \operatorname{val}(\tau^{-b_r})+\langle -u,a_r\rangle
  =-b_r-\langle a_r,u\rangle
  =-\ell_r(u).
\]
\begin{proposition}[Attention provenance as an initial degeneration]
\label{prop:attention-provenance-initial-degeneration}
With the max-plus score convention above, a term $\tau^{-b_r}\chi^{a_r}$ appears in $\operatorname{in}_{-u}(f)$ if and only if $r\in\argmax_s\ell_s(u)$.  Hence the initial form records exactly the active attention provenance for the rationalized finite head.  The tropical hypersurface is the locus where at least two candidates are retained; unique-max chambers are not in the hypersurface, but they are cells of the same decision stratification.
\end{proposition}
\begin{proof}
In the usual nonarchimedean min convention, $\operatorname{in}_{-u}(f)$ keeps those terms whose value
\[
  \operatorname{val}(\tau^{-b_r})+\langle -u,a_r\rangle
\]
is minimal.  The displayed sign identity identifies this value with $-\ell_r(u)$.  Minimizing $-\ell_r(u)$ is equivalent to maximizing $\ell_r(u)$, so the retained monomials are precisely the active max-plus candidates.  A tie gives a non-monomial initial form and therefore lies on the tropical hypersurface; a unique maximum gives a monomial initial form but still records a stable provenance chamber.
\end{proof}

\begin{definition}[Finite TropicalGT toric sidecar]
A finite TropicalGT toric sidecar is
\[
  \mathsf S_{\mathrm{TGT}}=(K,N,M,A,b,P,\mathcal F_H,I_H,\Sigma,\eta),
\]
where $A=(a_1,\ldots,a_R)$ and $b$ are rationalized candidate data, $\mathcal F_H$ is the finite Laurent-polynomial family exported by selected heads, coordinates, or graph-token windows, $I_H=\langle\mathcal F_H\rangle$ when an ideal sidecar is enabled, $\Sigma$ is a rational fan refining the observed active normal decomposition and any finite Groebner decomposition used by $I_H$, and $\eta$ records the lattice basis, denominator scale, valuation convention, max-plus/min-initial sign, software backend, and source checkpoint/window.
\end{definition}

Operationally, $\Sigma$ is chosen after export, not learned as an unrestricted neural object.  For a single Laurent polynomial it refines the normal fan of the lifted Newton polytope $\Conv\{(a_r,b_r)\}$ together with the active candidate cells observed in the batch.  For an ideal sidecar it also refines the finite Groebner cells that the CAS backend certifies or samples for $I_H$.  After this refinement, every cone of $\Sigma$ labels a constant initial-form and provenance regime on the audited sidecar; one dimensional cones carry the incidence data used for bend, Cartier-compatibility, and filtration checks.

For an ideal sidecar, set $Y_H=V(I_H)\cap T_N$ and
\[
  \operatorname{Trop}(Y_H)=\{u\in N_\mathbb R:\operatorname{in}_{-u}(I_H)\text{ contains no monomial}\}.
\]
If $|\Sigma|=\operatorname{Trop}(Y_H)$ and the multiplication map $T_N\times\overline Y_H\to X_\Sigma$ is flat and surjective, then the closure in $X_\Sigma$ is a tropical compactification in the sense of Tevelev \cite{tevelev2007compactifications}; smoothness of this map gives the stronger sch\"on case.  TropicalGT-I does not assume those hypotheses for ordinary training batches.  Without them, $X_\Sigma$ is a finite toric audit envelope: it provides orbit labels, one dimensional cone incidence, initial ideals, active-face refinements, bend data, and local resolution targets, but it does not certify global compactification, smooth boundary behavior, or sheaf cohomology of a neural model.

This sidecar also explains which exact methods are available.  Given the exponent matrix $A=[a_1,\ldots,a_R]$, the toric ideal is the kernel of
\[
  \varphi_A:K[x_1,\ldots,x_R]\longrightarrow K[M],\qquad x_r\longmapsto \chi^{a_r},
\]
namely
\[
  I_A=\ker\varphi_A=\langle x^\alpha-x^\beta:A\alpha=A\beta\rangle.
\]
This is the algebraic source of binomial consistency checks, Markov-basis moves, Groebner bases, monomial initial ideals, Hilbert series, Betti tables, and free-resolution targets.  Miller--Sturmfels two-variable monomial staircases give the correct picture for local two-parameter persistence slices and adjacent-generator first syzygies \cite{miller2005}; balanced tropical cycles give Minkowski weights and Chow classes only when the tropical-ideal or classical-tropicalization hypotheses are certified \cite{maclaganrincon2018tropicalideals,maclaganrincon2020balanced,fultonsturmfels1994intersection,katz2009tropicalintersection}.

The current implementation exposes this exact finite sidecar through a Macaulay2 certificate boundary.  Given an explicit integer exponent matrix, \texttt{cas\_toric} calls the Macaulay2 \texttt{Quasidegrees} method \texttt{toricIdeal(A,R)} and records the returned binomial kernel ideal, generator text, backend attempt, command template, and input hash.  The certificate is safe only as a finite monomial-map toric-ideal certificate: it is not a normal-fan certificate, not a tropical-variety certificate, and not evidence that the neural chart bundle is globally toric.  Chart-bundle logits, toric active rows, GraphCG cells, support tokens, and embeddings are therefore serialized as uncertified activation diagnostics unless this finite CAS sidecar or a separate tropical-fan certificate is attached.

For a certified fan $\Sigma$, the Cox ring and irrelevant ideal provide the sheaf-facing audit interface
\[
  S_\Sigma=K[x_\rho:\rho\in\Sigma(1)],\qquad
  B_\Sigma=\left\langle\prod_{\rho\not\preceq\sigma}x_\rho:\sigma\in\Sigma\right\rangle.
\]
Equivariant sheaf or vector-bundle diagnostics are therefore represented as graded Cox-module, local-cohomology, Fitting-ideal, or Klyachko-filtration sidecars only after the fan and grading data are exported.  In the current codebase, no Cox-module, local-cohomology, or vector-bundle wrapper is promoted to certified status yet: the browser and training metrics must render these fields as unavailable with an exact reason until a real CAS report supplies the fan, grading, module presentation, backend attempt, and certificate hash.  Sage, Normaliz, Gfan, Polymake, and Macaulay2 sidecars may compute these objects offline when explicitly configured, but none is a fallback for another and none may be replaced by activation rows, chain ranks, support tokens, or embedding similarities.  The GPU receives compact tensors such as active labels, multiplicities, Betti vectors, sparse differentials, filtration defects, or unavailable masks \cite{m2tropical2019}.  A differentiable loss is allowed only after the exact small-window sidecar validates the corresponding target, and it remains a late-phase auxiliary behind BPB and graph-BPB gates.
...
The same rule is applied to the OpenAI Parameter-Golf data stream.  Official FineWeb binary shards are read lazily as token windows, decoded with the local SentencePiece tokenizer when available, and converted into causal sequential DAGs before batching.  Thus the OAI baseline data are not a separate non-graph modality: each byte-prediction example has a graph \(G\), and the TokenGT encoder receives graph, vertex, edge, endpoint, and mask tensors.  The autoregressive byte objective is graph-aware.  If \(G\) is a causal DAG, node payloads are flattened in topological order; if \(G\) contains a cycle or explicitly non-causal edge, the implementation uses a deterministic seeded random node order.  This keeps the objective autoregressive without pretending that an undirected or cyclic graph carries a privileged causal direction.
...
The cap-sized implementation reuses this same tropical ring attention module in two places.  For graph tokens, it uses exact blockwise Hilbert scoring and max-plus aggregation to avoid materializing the full \((B,N,N,d)\) difference tensor on large real graphs.  For long byte contexts, it first pools the sequence into a bounded number of context chunks, applies the same tropical head blockwise to the pooled chunks, and broadcasts the resulting max-plus context as a residual before the recurrent byte predictor.  No second tropical attention module is introduced, so the long-context tropical path changes execution and metrics without consuming additional Parameter-Golf artifact bytes.
...
The GraphCG auxiliary follows Liu et al.\ \cite{liu2024graphcg}: learned directions \(d_1,\ldots,d_m\) define steerable latent edits in pooled graph state space.  The implementation uses a same-condition contrastive term over direction-shifted states, plus orthogonality, sparsity, and an explicit full-rank singular-value barrier.  If \(D\in\mathbb R^{m\times d}\) is the row-normalized direction matrix and \(r=\min(m,d)\), the full-rank term penalizes
\[
  \frac1r\sum_{i=1}^r \max\{0,\eta-\sigma_i(D)\}^2,
\]
...
\subsection{Multiparameter persistence, derived signatures, and analogical memory}
The current audit layer attaches a finite filtered simplicial object to each reasoning step and to the cumulative graph-of-thought trajectory.  Vertices represent graph or reasoning states, edges represent incidence or parent-child transitions, and two-simplices represent directed length-two reasoning paths.  For a bounded complex \(K\), the implementation computes chain groups over \(\mathbb F_2\), boundary ranks, Betti vectors, Euler characteristic, scalar persistent homology when a backend such as Gudhi is present, Stanley--Reisner degree-two nonfaces, Taylor-resolution upper bounds, DG-commutative cochain ranks, cup-product support counts, and a compact derived-equivalence signature.

For multiparameter persistence, the v1 implementation follows the algebraic dictionary used in multiparameter persistent homology \cite{schreiber2022multiparameter}: a finite \(n\)-parameter persistence module is represented as a multigraded module over \(\mathbb F_2[x_1,\ldots,x_n]\).  The implemented bounded report uses three parameters: filtration value, simplex dimension, and vertex-position scale.  It records multigraded generators for chain modules, monomial-labeled boundary maps, a finite grid of fiber ranks, sampled comparable-pair rank invariants, and a finite chain-presentation summary; certified free resolutions are reported only when a CAS backend supplies them.  If a dedicated package such as \texttt{multipers} \cite{loiseaux2024multipers} is installed, it can be used as a backend for richer signed-measure or invariant computations; otherwise the repo uses this exact bounded finite-field fallback.  This is enough for smoke-scale audits and for training-time correlations: each algebraic summary can be tested against BPB, graph-BPB, verifier score, and reward stability.

The analogical-reasoning component generalizes the categorical analogy perspective studied in transformer reasoning \cite{minegishi2026analogical}.  A memory item is not merely a vector.  It is a packet
\[
  M=(e,\Sigma_K,\rho_K,\partial_K,\mu_K,\Gamma_K),
\]
where \(e\) is a pooled embedding, \(\Sigma_K\) is the trajectory graph on reasoning states, \(\rho_K\) is the filtered simplicial object, \(\partial_K\) is the chain differential, \(\mu_K\) records selected multiplicative or cup-product data, and \(\Gamma_K\) is a derived signature containing Betti, persistence, rank-invariant, and graph statistics.  Retrieval scores combine embedding cosine similarity, signature cosine similarity, and a quality score derived from reward and NLL.  Thus memory retrieval is analogical in two senses: latent vectors must be nearby, and the algebraic-topological shape of the reasoning trajectory must be comparable.  The trainable memory head projects pooled graph states into the query space; its contrastive loss is ablated separately and is promoted only if it improves ordinary BPB or graph-BPB.


\subsection{TropicalGT chart bundles and scoped exponent-chart training}
\label{subsec:bundle-toric-training}

TropicalGT-I uses persistence-landscape vectors as optional geometric evidence for retrieval.  Two complementary tropical vector bundle theories give a more structural way to decide how such evidence should be transported, compared, and regularized across graph charts.  The first is the tropical toric theory of Khan and Maclagan, where a toric tropical vector bundle is encoded by a valuated matroid plus compatible one dimensional cone filtrations.  The second is the semiring-scheme theory of Jun, Mincheva, and Tolliver, where vector bundles are locally free modules over tropical schemes and transition functions are forced, under zero-sum-free hypotheses, to be monomial matrices.  For TropicalGT-I this suggests a practical design rule: local graph charts may carry different tropical coordinates, but feature transports between charts should be sparse monomial rescalings together with matroidal filtration constraints rather than arbitrary dense affine maps.  The finite toric sidecar of \Cref{subsec:finite-tropical-toric-sidecars} supplies the exact audit path for this design rule; the chart-bundle losses below are differentiable companions, not substitutes for CAS-certified small-window checks.

\paragraph{Reference-theory background.}
Khan and Maclagan's tropical toric vector bundles are not merely collections of piecewise-linear functions.  Their basic datum is a simple valuated matroid $M$ on a ground set $G_M$ together with decreasing flat filtrations $E^i(j)$ along the one dimensional cones of a fan; the vector-bundle condition requires every maximal cone to admit a matroid basis whose threshold joins recover those flats.  Their Cox-semirings and bend-congruence presentation explain why fibers are tropical linear spaces and why realizability depends on both the valuated matroid and the chosen filtrations.  Kaveh and Manon's toric-vector-bundle viewpoint adds the valuation/tropical-object interpretation of Klyachko-style data \cite{kavehmanon2023toricbundles}.  Jun, Mincheva, and Tolliver give the complementary semiring-scheme warning: over zero-sum-free local semirings with only trivial idempotent pairs, invertible changes of basis are monomial and many vector bundles split as line-bundle sums.  TropicalGT-I uses these results as finite compatibility templates and transport constraints, while treating expressive neural chart covers as metric-gated training constructions rather than literal tropical schemes.

\begin{definition}[TropicalGT chart bundle]
\label{def:tropicalgt-chart-bundle}
Let $G=(V,E,x)$ be a token graph with local charts $U_a\subseteq G$ produced by the graph encoder, and let $F_a\in\R_{\max}^{n_a\times r}$ be the hidden tropical feature table on chart $U_a$.  A rank-$r$ TropicalGT chart bundle is the tuple
\[
  \mathcal{E}_{\theta}(G)=(M,p,\{\mathcal{F}^{a,i}(j)\}_{a,i,j},\{T_{ab}\}_{U_a\cap U_b\neq\emptyset}),
\]
where $M$ is a valuated matroid on a ground set of feature atoms, $p$ is its circuit valuation, $\mathcal{F}^{a,i}(j)$ is a decreasing filtration of flats indexed by the tropical one dimensional cone probe or graph statistic $i$ in chart $a$, and each overlap transport is a tropical monomial map
\[
  T_{ab}(z)_k = c_{ab,k}+z_{\pi_{ab}(k)}.
\]
The bundle is chart-consistent on $G$ if $T_{bc}T_{ab}=T_{ac}$ on triple overlaps up to a tropical scalar and if every active filtration can be recovered as a join of atoms selected by tropical one dimensional cone inequalities.
\end{definition}

The definition mirrors the toric compatibility condition for tropical toric vector bundles: over each maximal cone, the one dimensional cone filtrations must be generated by a matroid basis.  In TropicalGT-I the cones are not literal fans of a toric variety unless they come from a certified finite sidecar of \Cref{subsec:finite-tropical-toric-sidecars}; otherwise they are local regions in activation space defined by graph neighborhoods, persistence summaries, and token context.  The toric condition is nevertheless useful because it gives an auditable finite constraint: for each chart, selected basis atoms must reconstruct the flats that determine which features are allowed to dominate.

\begin{definition}[Scoped max-linear exponent-chart sidecar for a reasoning state]
\label{def:tropicalgt-toric-embedding}
Let $A\in\Z^{m\times d}$ be an exponent matrix whose rows are graph, token, and topological probes, and let $h\in\R^d$ be a hidden state.  The associated scoped max-linear exponent-chart map is
\[
  \Phi_A(h)_u = \max_{\alpha\in A_u}\langle \alpha,h\rangle + b_{u,\alpha},\qquad u=1,\ldots,m.
\]
For a graph chart $U_a$, $\Phi_A$ is compatible with the chart bundle when the active rows of $A$ refine the chart filtration $\mathcal{F}^{a,i}(j)$ and when overlap transports preserve the induced exponent-chart activation cells.  These cells are not normal-fan cells unless a separate certified fan or tropical sidecar supplies that structure.
\end{definition}

This definition turns finite monomial-coordinate algebra into a trainable interface without asserting a full tropical-variety embedding into a toric variety.  The exponent rows are small integer templates for edge cuts, motif counts, persistence-landscape bins, register states, and token-window statistics.  Their max-linear images produce scoped tropical coordinate diagnostics in which local feature comparisons are cheap and in which chart changes can be checked by sparse permutations and shifts.  When the rows and biases are rationalized, the same data define a finite Laurent-polynomial sidecar; when a family of rows is exported, the induced toric ideal gives exact binomial, Groebner, Hilbert-series, Betti-table, and free-resolution checks for the finite monomial map.  These checks belong to the audit envelope unless the compactification and flatness/smoothness hypotheses of \Cref{subsec:finite-tropical-toric-sidecars} are separately certified.

\begin{definition}[One dimensional cone filtration atomization for graph tokens]
\label{def:one-dimensional-cone-filtration-atomization}
Let $P_i$ be graph-token probes such as endpoint incidence, edge type, active tropical support, GraphCG direction activity, persistence-bin membership, or memory-retrieval availability.  A one dimensional cone filtration atomization maps each probe to a one dimensional cone score $s_i(u)$ for feature atom $u\in G_M$ and defines
\[
  \mathcal{F}^{a,i}(j)=\operatorname{cl}_M\{u\in G_M:s_i^a(u)\ge j\},
\]
where $\operatorname{cl}_M$ is matroid closure.  The resulting chart filtration is admissible for TropicalGT-I when it is decreasing in $j$, has finite support on the sampled threshold grid, and is compared to basis-generated joins through $\mathcal{L}_{\mathrm{flat}}$.
\end{definition}

\begin{proposition}[Finite auditability of one dimensional cone filtration losses]
\label{prop:finite-one-dimensional-cone-auditability}
For a fixed batch, a finite atom set $G_M$, and a finite threshold grid, the losses $\mathcal{L}_{\mathrm{flat}}$, $\mathcal{L}_{\mathrm{toric}}$, and $\mathcal{L}_{\mathrm{GraphCG\mbox{-}toric}}$ are computable from finite incidence tables, chart ids, active tropical supports, GraphCG direction scores, and persistence or memory availability masks.  No unobserved continuum of toric points is required for the training audit.
\end{proposition}

\begin{proof}
All sets in Definition~\ref{def:one-dimensional-cone-filtration-atomization} are finite on a batch: graph tokens are finite, feature atoms are finite, and the threshold grid is finite.  Matroid closure and rank-defect penalties are therefore table operations on subsets of $G_M$.  The toric active-cell terms depend on finitely many max-linear probe rows, and the GraphCG term depends on finitely many learned directions.  Persistence and memory terms enter only through stored finite summaries or availability masks.  Thus the audit evaluates a finite list of ranks, closures, active rows, and distances.
\end{proof}

\begin{theorem}[Monomial transport principle for TropicalGT-I]
\label{thm:monomial-transport-principle}
Assume the local coordinate semirings of two overlapping TropicalGT charts are zero-sum-free and have only trivial idempotent pairs, and assume hidden feature modules are locally free of rank $r$.  Then every invertible chart transition that is linear over the tropical semiring is a tropical monomial matrix: each output feature is a permutation of one input feature plus a tropical unit shift.  Consequently, imposing monomial transports does not remove any invertible tropical-linear chart changes available under these hypotheses.
\end{theorem}

\begin{proof}[Proof sketch]
The semiring-scheme result of Jun, Mincheva, and Tolliver shows that bases of finite free modules over such semirings are unique up to rescaling and permutation.  Equivalently, an invertible matrix over the local semiring has exactly one nonzero unit in each row and column.  Applying this local statement to the free hidden-feature module on each chart gives the claimed form of $T_{ab}$.  The TropicalGT-I restriction is therefore not an ad hoc sparsity prior: it is the natural transition class for locally free tropical feature modules.\end{proof}

\begin{theorem}[Matroid-filtration compatibility loss]
\label{thm:matroid-filtration-compatibility}
Let $M$ be a valuated matroid with ground atoms $G_M$, and let each chart $a$ predict flats $\mathcal{F}^{a,i}(j)$.  If for every chart $a$ there exists a basis $B_a$ of $M$ such that
\[
  \mathcal{F}^{a,i}(j)=\bigvee_{u\in B_a:\,\langle u,v_i\rangle\ge j} u
\]
for all active one dimensional cone probes $i$ and thresholds $j$, then the chart bundle satisfies the toric tropical vector-bundle compatibility condition on the fan cells represented by the charts.  A nonnegative relaxation is
\[
  \mathcal{L}_{\mathrm{flat}} = \sum_{a,i,j} d_{\mathrm{flat}}\left(\mathcal{F}^{a,i}(j),\bigvee_{u\in B_a:\,s_{a,i}(u)\ge j}u\right),
\]
where $d_{\mathrm{flat}}$ is a rank-defect or incidence-distance penalty.
\end{theorem}

\begin{proof}[Proof sketch]
Khan and Maclagan define tropical toric vector bundles by exactly this compatibility: on each maximal cone, the filtrations along the one dimensional cones are generated from a matroid basis by thresholding the one dimensional cone pairings and joining the selected rank-one flats.  Replacing a maximal cone by an activation chart gives the displayed condition.  The loss is zero precisely when the predicted flats agree with a basis-generated family on the sampled chart.  Rank-defect penalties are valid relaxations because flats are determined by closure and rank in the underlying matroid.\end{proof}

\begin{proposition}[BPB-oriented chart consistency]
Let $q_{\theta}(B_t\mid C_t,a)$ be the next-byte distribution produced in chart $a$ for context $C_t$, and let $T_{ab}$ be a monomial transport on an overlap.  Suppose the transported logits satisfy
\[
  \norm{\ell_{\theta}(C_t,a)-\ell_{\theta}(T_{ab}C_t,b)}_{\infty}\le \epsilon
\]
for all sampled overlaps.  Then the induced BPB gap between the two chart predictions is bounded by $\epsilon/\log 2$ plus the sampling error of the overlap estimator.
\end{proposition}

\begin{proof}[Proof sketch]
Changing all logits by at most $\epsilon$ changes the log-partition and the selected byte logit by at most $\epsilon$ each, so the negative log-likelihood changes by at most a constant multiple of $\epsilon$.  Expressing the result in bits divides by $\log 2$.  The empirical objective estimates this overlap expectation by sampled chart pairs.\end{proof}

\paragraph{Training objectives.}
The theory motivates four losses that are compatible with the existing BPB-first objective.
\begin{align*}
\mathcal{L}_{\mathrm{transport}} &= \sum_{(a,b)} \norm{F_b - T_{ab}(F_a)}_1,\\
\mathcal{L}_{\mathrm{cocycle}} &= \sum_{(a,b,c)} \norm{T_{bc}T_{ab}-T_{ac}}_{\mathrm{mono}},\\
\mathcal{L}_{\mathrm{toric}} &= \sum_{a}\mathrm{dist}\bigl(\operatorname{active}(\Phi_A(F_a)),\operatorname{normalFanCell}(\mathcal{F}^{a})\bigr),\\
\mathcal{L}_{\mathrm{BPB\mbox{-}chart}} &= \sum_{(a,b,t)}\left|\operatorname{NLL}_{a,t}-\operatorname{NLL}_{b,t}^{T_{ab}}\right|_{\delta}.
\end{align*}
The total training criterion is
\[
\mathcal{L}=\mathcal{L}_{\mathrm{byte}}+\lambda_g\mathcal{L}_{\mathrm{graph\mbox{-}BPB}}+\lambda_T\mathcal{L}_{\mathrm{transport}}+
\lambda_C\mathcal{L}_{\mathrm{cocycle}}+\lambda_F\mathcal{L}_{\mathrm{flat}}+
\lambda_A\mathcal{L}_{\mathrm{toric}}+
\lambda_B\mathcal{L}_{\mathrm{BPB\mbox{-}chart}}.
\]
The auxiliary coefficients should be warm-started at zero and increased only when validation BPB, graph BPB, and certificate losses remain stable.

\begin{center}
\scriptsize
\begin{tabular}{@{}p{0.18\linewidth}p{0.30\linewidth}p{0.24\linewidth}p{0.20\linewidth}@{}}
\toprule
Object & Exact sidecar & Differentiable companion & Failure mode \\
\midrule
Initial forms & rational $\operatorname{in}_{-u}(\mathcal F_H)$ or $\operatorname{in}_{-u}(I_H)$ & active-support agreement, margin, wall-hit loss & unavailable if valuation or sign metadata disagree \\
Fan and bends & normal fan refinement, one dimensional cone incidence, Cartier compatibility & cell-distance and bend-matching losses & audit envelope only if refinement or Cartier data are incomplete \\
Toric ideals and staircases & Macaulay2 \texttt{toricIdeal(A,R)} certificate, monomial initial ideal, two-variable staircase, Hilbert series, Betti table, free resolution & binomial and resolution residuals & skip active loss when CAS limits or exactness checks fail \\
Chow/Minkowski data & balanced multiplicities or certified Minkowski weights & balance and Chow residuals & visualization only unless balancing hypotheses are certified \\
Bundle/sheaf data & Klyachko-style filtrations, Cox modules, local cohomology or Fitting ideals when available & flat, cocycle, gluing, rank-defect losses & late-phase auxiliary after toy-window certification \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{Metric-facing implementation contract.}
The geometric machinery is accepted only through implementation metrics that already govern TropicalGT-I.  The byte objective remains primary, so any vector-bundle or toric-embedding term must report its effect on validation BPB and graph-BPB before promotion.  The chart-consistency regularizers report overlap transport error, cocycle defect, flat rank defect, and chart NLL gap; these metrics make the tropical chart structure auditable rather than decorative.  Persistence-landscape memory comparison reports availability, transported L2 distance, and transported cosine distance only for real GUDHI $\lambda_k(t)$ vectors, while unavailable vectors remain masked.  Optional inference artifacts should therefore expose chart ids, transport ids, toric active rows, and top-$k$ memory landscape distances without changing decoding unless the corresponding validation BPB or graph-BPB gate has passed.  Normal-fan cell labels are displayed only when a certified fan/tropical sidecar is attached; otherwise the artifact must retain the explicit uncertified activation-chart status.

\paragraph{Parameter-Golf accounting for bundle geometry.}
The Parameter-Golf/OAI path treats bundle and toric quantities as training-time side information unless explicitly exported.  A candidate auxiliary is therefore evaluated in three modes: coefficient zero, telemetry-only, and active-loss.  The stripped contest artifact may retain only the baseline byte model, TokenGT graph adapter, and compressed weights; chart ids, transport matrices, matroid atoms, and browser diagnostics remain research artifacts unless their byte cost is included in the manifest.  This prevents an auxiliary from appearing to improve BPB by smuggling extra graph or memory information into the deployed artifact.  The acceptance rule is
\[
  \Delta_{\mathrm{aux}}=\mathrm{BPB}_{\mathrm{val}}(\theta_{\mathrm{active}})-\mathrm{BPB}_{\mathrm{val}}(\theta_{\mathrm{zero}}),
  \qquad
  \Delta^{G}_{\mathrm{aux}}=\mathrm{BPB}_{\mathrm{graph,val}}(\theta_{\mathrm{active}})-\mathrm{BPB}_{\mathrm{graph,val}}(\theta_{\mathrm{zero}}),
\]
with promotion only when both deltas are nonpositive within tolerance and artifact-byte accounting is unchanged or explicitly charged.


\begin{definition}[TokenGT tropical atlas]
\label{def:tokengt-tropical-atlas}
For a TokenGT graph-token batch, a TokenGT tropical atlas is a finite family of charts
\[
  U_a=(S_a, C_a, \tau_a, \Gamma_a),
\]
where $S_a$ is a subset of graph, vertex, edge, and byte-context tokens; $C_a$ is the active max-plus support cell of the tropical ring attention head on those tokens; $\tau_a$ is a graph-of-thought state or partial trajectory; and $\Gamma_a$ is the derived topological signature attached to that state.  Overlaps are sampled when two charts share graph tokens, GFlowNet trajectory prefixes, GraphCG direction neighborhoods, or persistence bins.  A chart bundle on this atlas uses the monomial transports and matroidal flat filtrations of Definition~\ref{def:tropicalgt-chart-bundle} to compare the same reasoning object across these local views.
\end{definition}

This atlas is an engineering analogue of the toric charts in the Khan--Maclagan setting, not a literal tropical toric variety.  The theorem-level transfer is the local algebraic form of invertible tropical-linear transitions and the matroid-filtration compatibility pattern.  The choice of chart cover, overlap sampling, and coefficient schedule is a training principle tied to TropicalGT-I metrics.

\begin{proposition}[Metric-gated auxiliary geometry]
Suppose a model-selection rule promotes a vector-bundle or toric-embedding auxiliary only when the same validation protocol reports no increase in $\mathrm{BPB}_{\mathrm{text}}$ and no increase in $\mathrm{BPB}_{\mathrm{graph}}$ relative to the current checkpoint.  Then the promoted checkpoint is empirically non-worse under the declared BPB gates, regardless of whether the auxiliary has a geometric interpretation.
\end{proposition}

\begin{proof}
The claim is a property of the selection rule rather than of the optimizer.  Promotion is conditioned on the measured inequalities for text BPB and graph-BPB on the fixed validation protocol.  If either inequality fails, the auxiliary is not promoted.  Thus any promoted checkpoint satisfies the advertised empirical gate by definition.  The geometric losses can still be useful for search, interpretability, or retrieval, but this proposition deliberately avoids claiming that they improve BPB without the validation check.
\end{proof}

\paragraph{Training principle: bundle-guided graph-of-thought search.}
A graph-of-thought trajectory is a path through pooled TokenGT states, and the GFlowNet policy assigns probability mass to expand, merge, refine, retrieve, verify, compress, and stop actions.  The bundle view adds local coordinates to each state: a chart id, a tropical support cell, a toric active-row pattern, and a matroid flat signature.  A coherent auxiliary reward is
\[
  R_{\mathrm{bundle}}(\tau)=R_{\mathrm{task}}(\tau)-\eta_1\sum_{(a,b)\in\tau}\mathcal{L}_{\mathrm{transport}}(a,b)-\eta_2\sum_{(a,b,c)\in\tau}\mathcal{L}_{\mathrm{cocycle}}(a,b,c)-\eta_3\sum_{a\in\tau}\mathcal{L}_{\mathrm{flat}}(a).
\]
This is not a theorem from tropical vector-bundle theory; it is a training principle.  The justification is that GoT branches that reuse information across charts should pay for inconsistent transports, while the GFlowNet still receives the task and BPB-derived reward that decides promotion.

\paragraph{Training principle: GraphCG directions as toric one dimensional cone probes.}
GraphCG learns latent steering directions.  The scoped exponent-chart sidecar can treat a small subset of stable, full-rank GraphCG directions as candidate one dimensional cone probes $v_i$ in the filtration terms $\mathcal{F}^{a,i}(j)$.  The corresponding regularizer penalizes disagreement between the active GraphCG direction cone and the active exponent-chart cell; a normal-fan cell comparison is shown only when a certified fan/tropical sidecar is attached:
\[
  \mathcal{L}_{\mathrm{GraphCG\mbox{-}toric}}=\sum_a d_{\mathrm{cell}}\left(\operatorname{cone}(D_{\mathrm{active}}(h_a)),\operatorname{cell}(\Phi_A(h_a))\right)+\gamma\,\mathcal{L}_{\mathrm{full\mbox{-}rank}}(D).
\]
The full-rank term remains the existing GraphCG diagnostic; the scoped exponent-chart term only asks whether the directions that steer a chart are also visible in its max-linear coordinates.

\paragraph{Training principle: stability-inspired atom balance.}
Khan and Maclagan's slope-stability theory for tropical toric vector bundles depends on flat lattices and modularity hypotheses that TropicalGT-I does not automatically satisfy.  The transferable part is a finite diagnostic: for a subset of atoms $H\subset G_M$, compare its average chart load to the load of the whole atom set,
\[
  \mu_a(H)=\frac{1}{\operatorname{rk}_M(H)+\eps}\sum_i\sum_j \omega_{ij}\,\mathbf{1}\{H\cap \mathcal{F}^{a,i}(j)\ne\emptyset\},\qquad
  \mathcal{L}_{\mathrm{atom\mbox{-}stability}}=\sum_a\sum_{H\in\mathcal{H}_a}\left[\mu_a(H)-\mu_a(G_M)+m\right]_+.
\]
Here $\mathcal{H}_a$ is a small sampled family of flats or atom clusters and $m$ is a margin.  This is not a stability theorem for the neural model; it is a BPB-gated regularizer intended to prevent all useful graph, GraphCG, persistence, or memory atoms from collapsing into a single overloaded flat.

\paragraph{Two-parameter persistence and analogical memory.}
The two-parameter persistence module already records how graph-of-thought simplices appear under two filtrations such as tropical margin and NLL.  Vector-bundle training does not turn this module into a toric vector bundle.  Instead it uses its summaries as chart labels and memory atoms.  Rank-invariant bins, Betti profiles, and optional GUDHI persistence landscapes can enter $G_M$ as atoms; analogical retrieval then compares both latent embeddings and transported topological atoms.  If a memory row lacks a GUDHI landscape vector, the corresponding atom is absent from the comparison.  This preserves the mathematical distinction between a persistence landscape $\lambda_k(t)$ and an NLL or fitness field on a graph-of-thought visualization.

\begin{example}[A chart-consistency batch]
Consider a byte window whose TokenGT graph contains problem, intermediate reasoning, and answer vertices.  Chart $U_a$ contains the edge tokens supporting the current answer, chart $U_b$ contains a retrieved analogical memory item with a matching derived signature, and chart $U_c$ contains a GraphCG-shifted variant used by the verifier.  The training step computes byte NLL and graph-BPB as usual.  It also computes $T_{ab}$ for retrieval reuse, $T_{bc}$ for the verifier branch, a cocycle defect against $T_{ac}$, a flat rank-defect for active GraphCG one dimensional cone probes, and transported L2/cosine distances for any real GUDHI landscape vectors.  The optimizer may use these quantities, but the model-selection rule accepts the change only if the validation BPB gates pass.
\end{example}

\paragraph{Example: transported persistence-landscape memory.}
For a sample with a GUDHI persistence landscape vector $\lambda=(\lambda_1(t_1),\ldots,\lambda_K(t_L))$, include $\lambda$ as a feature atom family in $G_M$.  If an analogical memory item is retrieved in chart $a$ but reused in chart $b$, compare transported vectors $T_{ab}\lambda_a$ and $\lambda_b$ by L2 and cosine distance.  Missing landscape vectors must remain unavailable: the matroid atom is masked, not filled by a fabricated zero landscape.  This separates GUDHI persistence landscapes from the GoT NLL or fitness field while still allowing both to influence retrieval through separate chart coordinates.

\subsection{Bits-per-byte and graph bits-per-byte}
...
\[
  \mathrm{BPB}_{\mathrm{text}}=\frac{\sum_t-\log p_\theta(b_t\mid b_{<t},G)}{\log 2\cdot |\{t:b_t\ne\mathrm{pad}\}|}.
\]
TropicalGT additionally reports graph-aware scores.  Let \(B\) be the number of predicted target bytes, \(S(G)\) the deterministic structural byte count of the actually tokenized graph tokens, and \(J(G)\) the explicit JSON byte count of non-derived graph side information after removing the deterministic text path.  Here \(S(G)\) is computed from live graph-token masks, node counts, and edge endpoint ids when endpoint tensors are supplied, so the graph-BPB denominator reflects the same TokenGT structure that conditions the model.  With side-information weight \(\rho\), the implementation logs
\[
  \mathrm{BPB}_{\mathrm{graph}}=\frac{\mathrm{NLL}_{\mathrm{bits}}+8\rho J(G)}{B+S(G)},\qquad
  \mathrm{BPB}_{\mathrm{side}}=\frac{\mathrm{NLL}_{\mathrm{bits}}+8\rho J(G)}{B}.
\]
It also logs the optimistic conditioning score \(\mathrm{NLL}_{\mathrm{bits}}/(B+S(G))\), which treats graph structure as already present in context.  These three numbers separate contest-style byte compression, graph-conditioned compression, and honest side-information accounting.  All tropical, GraphCG, GFlowNet, persistence, and analogical-memory metrics are therefore evaluated by whether they correlate with, or causally improve under ablation, \(\mathrm{BPB}_{\mathrm{text}}\) and \(\mathrm{BPB}_{\mathrm{graph}}\).  The stripped Parameter-Golf export path keeps this accounting separate from the research stack: it packages only the baseline training script, the TokenGT graph adapter, the compressed model artifact, and a manifest that checks code-plus-artifact bytes against the local \(16{,}000{,}000\)-byte cap and records the BPB/graph-BPB contract.

The current cap-sized configuration uses width and hidden size \(1760\), analogical-memory width \(220\), sequence length \(1024\), batch size \(80\), and 5K-step checkpoints.  The local estimator instantiates the configured model, applies the same int8 state-dict quantization and zlib compression used by the stripped Parameter-Golf exporter, adds the stripped baseline code bytes, and checks the result against the \(16{,}000{,}000\)-byte cap.  In the most recent implementation pass this estimate was approximately \(15.63\) MB, leaving a small safety margin while keeping the research model large enough to use the available accelerator memory during training.

\subsection{Training, evaluation, inference, and visualization contract}
The runnable contract is implemented by the training, evaluation, inference, validation, visualization, BPB-ablation, readiness-audit, export-size-estimation, and periodic BPB review-loop drivers.  Data-backed configurations require the moved ToricGT parquet shards explicitly and may also include optional OAI Parameter-Golf binary shards.  The loader first builds row-group or shard metadata, reads examples through bounded caches or memmaps rather than concatenating corpora into memory, and refuses to fall back to fixtures when required data are missing.  The full training configuration uses a deterministic weighted hybrid of TropicalGT reasoning shards and OAI graph-window records; validation reports graph-token statistics, graph-json fallback rate, causal-DAG versus random-order autoregressive rates, OAI-source rate, node/edge statistics, and sample graph-token descriptors before long training begins.  The readiness audit script combines environment/package checks, required-data manifest checks, sample TokenGT conversion, deterministic sequential-text graphification, paper artifact checks, checkpoint reload, finite evaluation, text-BPB and graph-BPB gates, bounded inference scaling, optional topological algebra audits, ablation-tool availability, and optional visualization generation into a single JSON/Markdown proof bundle.  For a full training configuration with no checkpoint yet, the same audit can run a bounded dry-run optimizer step on CUDA: it builds the configured model, samples hybrid graph records, executes forward/backward/update, verifies the W\&B key path, and gates finite loss, gradient norm, text BPB, graph-BPB, and GPU allocation before the long job is launched.  Training reports language-model NLL, exact text BPB, graph-BPB, graph side-information BPB, GFlowNet trajectory-balance loss, GraphCG contrastive and full-rank direction geometry terms, finite graph-certificate loss and agreement, graph-token and pooled-sequence tropical support entropy, tropical margins, wall-hit rate, graph-token statistics, graph JSON fallback rate, graph JSON sequentialization rate, causal/random graph-AR rates, OAI-source rate, multiparameter persistence summaries, analogical-memory query norms, examples/sec, tokens/sec, GPU memory, and step throughput to W\&B when enabled.  The 5K review loop trains to each boundary, evaluates the configured BPB target, and writes an active training contract containing every active metric family, hyperparameter, loss weight, objective, regularizer, data-source rate, VRAM/throughput statistic, visualization path, and restart decision.  If held-out \(\mathrm{BPB}_{\mathrm{text}}\) is missing or above the target, the loop marks the boundary for human or automated review and restart according to the chosen policy.

The ablation analyzer consumes one or more training reports, ranks logged metrics by correlation with \(\mathrm{BPB}_{\mathrm{text}}\), \(\mathrm{BPB}_{\mathrm{graph}}\), and held-out evaluation variants, and emits best-by-target summaries with the lowest run, baseline delta, runner-up, and runner-up margin for each compression target; the ablation-grid runner generates same-seed loss-weight variants such as no-GraphCG, no-GFlowNet, no-certificate, no-tropical-regularizer, no-auxiliary, half/quarter auxiliary weights, and isolated low-weight GFlowNet, GraphCG, and tropical-regularizer settings, then optionally trains them and runs the analyzer.  These reports are screening devices for matched ablations, not causal evidence by themselves.  The first finite certificate objective is deliberately modest: an edge token is structurally certified when its active tropical support is the edge token itself or one of its endpoint vertex tokens.  This is a TokenGT skeleton certificate rather than a semantic proof; it should be promoted only when it improves held-out BPB, verifier score, support faithfulness, or downstream reasoning metrics.  Checkpoints store model state, optimizer state, step, metric history, config, and random-number-generator state; long runs can resume from either the final checkpoint or a periodic \texttt{.latest.pt} checkpoint.  Evaluation reports aggregate NLL/BPB/graph-BPB together with optional per-record NLL, tropical support histograms, graph-token traces, filtered simplicial objects, and opt-in topological algebra audits.  Inference emits the generated byte-level argmax string plus BPB accounting, tropical support/margin diagnostics, GFlowNet action probabilities, GraphCG direction summaries, a graph-token trace, the filtered object for the prompt graph, optional multiparameter algebra, optional analogical-memory retrieval, and an optional bounded inference-scaling report containing all expanded candidates and the selected graph-of-thought path.  The visualization script projects graph-of-thought candidate embeddings by PCA and writes dark-mode interactive three-dimensional trajectories whose edges are the reasoning graph.  Hovering over a node renders a cursor-following SVG hover card and updates a persistent SVG side panel for that node's filtered simplicial object, so the audit shows actual vertices, edges, two-simplices, filtration colors, and filtration layers rather than only textual summaries.  The same HTML stores the complete object in JSON, and the PCA/NLL view adds a smoothed heatmapped NLL surface under the reasoning graph or as the NLL-height field.  Additional Plotly artifacts visualize tropical support heatmaps, persistence barcodes, persistence-module Betti profiles, GraphCG direction cosines, GraphCG full-rank singular spectra, GraphCG training direction geometry, analogical memory retrieval scores, and training metrics.  In the first implementation each filtered object contains vertices as \(0\)-simplices, graph edges as \(1\)-simplices, directed length-two reasoning paths as \(2\)-simplices, filtration thresholds, and a compact summary; longer runs can replace the deterministic smoke filtration with learned margin, verifier, or persistence-based weights.

This implementation deliberately omits toric-only mechanisms from the deployable TropicalGT-I student unless they admit a tropical meaning and pass the BPB gates.  Ported methods are the dataset schema, graph-of-thought trajectory view, GFlowNet branch training, GraphCG steering, W\&B discipline, reproducibility reports, latent-trajectory visual audits, and finite algebraic sidecars for rationalized tropical probes.  Noncommutative toric phase memory and torus-specific chamber constraints remain out of scope for the first TropicalGT-I artifact.  Toric ideals, Cox modules, Chow/Minkowski weights, and Klyachko-style filtrations enter only as opt-in finite audit sidecars or late-phase auxiliary losses after exact small-window certification; the always-on tropical replacements remain active faces, Hilbert margins, max-plus supports, and fan-cell certificates.
...
\subsection{Vector-bundle and toric-embedding training recipe}
\label{app:vector-bundle-toric-recipe}

\begin{lstlisting}[language=Python,caption={Tropical vector-bundle and toric-embedding auxiliary losses.}]
def vector_bundle_toric_step(batch, model, memory, coeffs):
    out = model(batch, return_charts=True, return_toric=True)
    byte_loss = out.byte_nll.mean()
    graph_bpb = graph_bits_per_byte(out.graph_nll, batch.num_bytes)

    charts = out.chart_features          # {chart_id: [nodes, rank]}
    overlaps = sample_chart_overlaps(batch.graph, charts)
    transports = model.monomial_transports(overlaps)

    transport_loss = 0.0
    cocycle_loss = 0.0
    bpb_chart_loss = 0.0
    for (a, b) in overlaps.pairs:
        Fa_to_b = transports[(a, b)].apply(charts[a])
        transport_loss += l1_on_overlap(Fa_to_b, charts[b], overlaps[(a, b)])
        bpb_chart_loss += huber(out.nll_by_chart[a] -
                                transports[(a, b)].apply_logits(out.nll_by_chart[b]))
    for (a, b, c) in overlaps.triples:
        cocycle_loss += monomial_cocycle_defect(transports[(a, b)],
                                                transports[(b, c)],
                                                transports[(a, c)])

    flats = model.predicted_flats(out.toric_scores)
    flat_loss = matroid_rank_defect(flats, model.bundle_matroid)
    toric_loss = normal_fan_cell_loss(out.toric_embedding, flats)

    landscape_loss = 0.0
    for item in memory.topk(batch.ids):
        if item.has_gudhi_landscape_vector:
            landscape_loss += transported_l2_cosine(item, transports)
        else:
            item.mark_landscape_unavailable()  # never fabricate a zero vector

    loss = (byte_loss + coeffs.graph * graph_bpb +
            coeffs.transport * transport_loss +
            coeffs.cocycle * cocycle_loss +
            coeffs.flat * flat_loss +
            coeffs.toric * toric_loss +
            coeffs.chart_bpb * bpb_chart_loss +
            coeffs.landscape * landscape_loss)
    return loss, {
        "train/bpb": to_bpb(byte_loss),
        "train/graph_bpb": graph_bpb,
        "bundle/transport_l1": transport_loss,
        "bundle/cocycle_defect": cocycle_loss,
        "bundle/flat_rank_defect": flat_loss,
        "bundle/atom_stability_gap": atom_stability_gap(flats, model.bundle_matroid),
        "toric/normal_fan_loss": toric_loss,
        "chart/bpb_consistency": bpb_chart_loss,
        "memory/landscape_available_rate": memory.landscape_available_rate(),
    }
\end{lstlisting}


\begin{lstlisting}[language=Python,caption={Metric-gated bundle and toric promotion.}]
def promote_bundle_geometry(baseline, candidate, val_batch, gates):
    base = evaluate_tropicalgt_i(baseline, val_batch,
                                 emit_optional_artifacts=False)
    cand = evaluate_tropicalgt_i(candidate, val_batch,
                                 emit_optional_artifacts=True)
    checks = {
        "bpb_ok": cand["val/bpb"] <= base["val/bpb"] + gates.bpb_tol,
        "graph_bpb_ok": cand["val/graph_bpb"] <= base["val/graph_bpb"] + gates.graph_bpb_tol,
        "cert_ok": cand["certificate/loss"] <= gates.max_certificate_loss,
        "wall_hit_ok": cand["tropical/wall_hit_rate"] <= gates.max_wall_hit_rate,
    }
    if all(checks.values()):
        return "promote", cand, checks
    return "anneal_or_disable_auxiliary", cand, checks
\end{lstlisting}

\paragraph{Implementation notes.}
The monomial transports can be parameterized by a Sinkhorn-relaxed permutation during warm-up and projected to a hard permutation for audits.  The toric exponent matrix $A$ should be small and integer-valued; useful rows include token-window counts, graph degree and cut probes, persistence-landscape bins, and register-activation indicators.  The browser should label GUDHI persistence landscapes as $\lambda_k(t)$ vectors and should label GoT loss or fitness fields separately as NLL, fitness, or density fields.
...
\begin{objectivecard}{37}{Tropical Vector-Bundle Feature Transport}{Define chart overlaps in the graph encoder and train sparse monomial transports that preserve byte logits, graph states, and optional GUDHI landscape atoms across charts.}{Theorem~\ref{thm:monomial-transport-principle} identifies monomial maps as the natural invertible transition functions for locally free tropical feature modules.}{Transported features should agree on overlaps without increasing validation BPB.}{transport L1, cocycle defect, overlap BPB gap, graph BPB}{Start with soft permutations, project only during audits, and disable the loss for charts with too few overlap nodes.}\end{objectivecard}

\begin{objectivecard}{38}{Matroid Filtration Compatibility}{Attach a small valuated matroid to feature atoms and penalize predicted chart filtrations that cannot be reconstructed from basis-generated flats.}{The toric vector-bundle compatibility theorem of Khan--Maclagan translates to a finite rank-defect objective on sampled TropicalGT charts.}{Hidden features become more stable under context perturbations and analogical retrieval.}{flat rank defect, basis coverage, certificate loss, retrieval stability}{Use tiny matroids first; large ground sets make basis search expensive.}\end{objectivecard}

\begin{objectivecard}{39}{Scoped Exponent-Chart Regularizer}{Train an integer max-linear exponent-chart sidecar whose active rows agree with chart filtrations and persistence summaries.}{Scoped tropical coordinates expose which graph and topological probes dominate a reasoning state without certifying a toric-variety embedding.}{Regularized charts should improve graph BPB and make browser audits more interpretable.}{activation-cell loss, active-row entropy, exponent margin, graph BPB}{Anneal slowly; over-regularization can collapse useful non-toric hidden directions.}\end{objectivecard}

\begin{objectivecard}{40}{BPB Chart Consistency}{On overlapping charts, minimize robust differences in byte NLL after monomial transport.}{The BPB consistency proposition bounds local chart disagreement by transported logit error.}{Auxiliary geometry is accepted only when it helps byte-level compression.}{overlap NLL gap, validation BPB, graph BPB, certificate loss}{Gate the objective off when certificate loss rises or tropical wall-hit rate spikes.}\end{objectivecard}
...
\bibitem{khan2024tropicalvectorbundles}
B. Khan and D. Maclagan.
\newblock Tropical vector bundles.
\newblock arXiv:2405.03505, 2024.

\bibitem{jun2023vectorbundlestropicalschemes}
J. Jun, K. Mincheva, and J. Tolliver.
\newblock Vector bundles on tropical schemes.
\newblock arXiv:2009.03030v2, 2023.
...
Motasem Alfarra, Adel Bibi, Hasan Hammoud, Mohamed Gaafar, and Bernard Ghanem.
\newblock On the decision boundaries of neural networks: a tropical geometry perspective.
...
\bibitem{tevelev2007compactifications}
Jenia Tevelev.
\newblock Compactifications of subvarieties of tori.
\newblock arXiv:math/0412329, 2005.

\bibitem{maclaganrincon2018tropicalideals}
Diane Maclagan and Felipe Rinc\'on.
\newblock Tropical ideals.
\newblock \emph{Compositio Mathematica}, 154(3):640--670, 2018. arXiv:1609.03838.

\bibitem{maclaganrincon2020balanced}
Diane Maclagan and Felipe Rinc\'on.
\newblock Varieties of tropical ideals are balanced.
\newblock arXiv:2009.14557, 2020.

\bibitem{m2tropical2019}
Carlos Am\'endola, Kathl\'en Kohn, Sara Lamboglia, Diane Maclagan, Ben Smith, Jeff Sommars, Paolo Tripoli, and Magdalena Zajaczkowska.
\newblock Computing tropical varieties in Macaulay2.
\newblock arXiv:1710.10651, 2019.

\bibitem{fultonsturmfels1994intersection}
William Fulton and Bernd Sturmfels.
\newblock Intersection theory on toric varieties.
\newblock arXiv:alg-geom/9403002, 1994.

\bibitem{katz2009tropicalintersection}
Eric Katz.
\newblock Tropical intersection theory from toric varieties.
\newblock arXiv:0907.2488, 2009.

\bibitem{kavehmanon2023toricbundles}
Kiumars Kaveh and Christopher Manon.
\newblock Toric vector bundles, valuations and tropical geometry.
\newblock arXiv:2304.11211, 2023.

\bibitem{schreiber2022multiparameter}
Amelie Schreiber.
\newblock Multiparameter persistent homology: generic structures and quantum computing.
\newblock \emph{arXiv:2210.11433}, 2022.

\bibitem{loiseaux2024multipers}
David Loiseaux and Hannah Schreiber.
\newblock \texttt{multipers}: multiparameter persistence for machine learning.
\newblock \emph{Journal of Open Source Software}, 9(103):6773, 2024.

\bibitem{minegishi2026analogical}
...
\newblock \emph{arXiv:2602.01992}, 2026.
