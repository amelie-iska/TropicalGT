...
\subsection{TropicalGT chart bundles and toric embedding training}
...
\begin{definition}[Tropical toric embedding of a reasoning state]
\label{def:tropicalgt-toric-embedding}
Let $A\in\Z^{m\times d}$ be an exponent matrix whose rows are graph, token, and topological probes, and let $h\in\R^d$ be a hidden state.  The associated tropical toric embedding is
\[
  \Phi_A(h)_u = \max_{\alpha\in A_u}\langle \alpha,h\rangle + b_{u,\alpha},\qquad u=1,\ldots,m.
\]
For a graph chart $U_a$, $\Phi_A$ is compatible with the chart bundle when the active rows of $A$ refine the chart filtration $\mathcal{F}^{a,i}(j)$ and when overlap transports preserve the induced normal fan cells.
\end{definition}

This definition turns toric geometry into a trainable interface.  The exponent rows are small integer templates for edge cuts, motif counts, persistence-landscape bins, register states, and token-window statistics.  Their max-linear images produce a toric tropical coordinate system in which local feature comparisons are cheap and in which chart changes can be checked by sparse permutations and shifts.  When the rows and biases are rationalized, the same data define a finite Laurent-polynomial sidecar; when a family of rows is exported, the induced toric ideal gives exact binomial, Groebner, Hilbert-series, Betti-table, and free-resolution checks.  These checks belong to the audit envelope unless the compactification and flatness/smoothness hypotheses of \Cref{subsec:finite-tropical-toric-sidecars} are separately certified.
...
GraphCG learns latent steering directions.  A tropical toric embedding can treat a small subset of stable, full-rank GraphCG directions as candidate one dimensional cone probes $v_i$ in the filtration terms $\mathcal{F}^{a,i}(j)$.  The corresponding regularizer penalizes disagreement between the active GraphCG direction cone and the active toric normal-fan cell:
\[
...
\]
The full-rank term remains the existing GraphCG diagnostic; the toric term only asks whether the directions that steer a chart are also visible in its max-linear toric coordinates.
...
\begin{objectivecard}{39}{Tropical Toric Embedding Regularizer}{Train an integer max-linear toric embedding whose active normal-fan cells agree with chart filtrations and persistence summaries.}{Toric tropical coordinates expose which graph and topological probes dominate a reasoning state.}{Regularized charts should improve graph BPB and make browser audits more interpretable.}{normal-fan loss, active-row entropy, toric margin, graph BPB}{Anneal slowly; over-regularization can collapse useful non-toric hidden directions.}\end{objectivecard}
