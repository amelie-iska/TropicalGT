...
\[
  ...
  =-\ell_r(u).
\]
Thus $\operatorname{in}_{-u}(f)$ keeps exactly the monomials indexed by $\argmax_r\ell_r(u)$.  The tie locus is the tropical hypersurface; the unique-max chambers remain part of the TropicalGT decision stratification because they record which proof alternative, predecessor, memory item, or graph-token witness was selected.
...
The definition mirrors the toric compatibility condition for tropical toric vector bundles: over each maximal cone, the one-dimensional-cone filtrations must be generated by a matroid basis.  In TropicalGT-I the cones are not literal fans of a toric variety; they are local regions in activation space defined by graph neighborhoods, persistence summaries, and token context.  The toric condition is nevertheless useful because it gives an auditable finite constraint: for each chart, selected basis atoms must reconstruct the flats that determine which features are allowed to dominate.
