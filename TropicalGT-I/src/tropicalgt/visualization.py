...
def _write_two_parameter_bifiltration_staircase_html(
    ...
    *,
    ...
    def _svg_chain_staircase(dim: int, items: Sequence[tuple[int, int, int, str]], *, primary: bool = False) -> str:
        ...
        width = 1180 if primary else 880
        height = 640 if primary else 360
        left = 92
        right = 64
        top = 62
        bottom = 84
        ...
        for lvl in range(0, int(y_axis_max) + 1):
            for rg in range(0, int(x_axis_max) + 1):
                ...
                color = "#141922" if inside else "#eef2f7"
                opacity = "0.52" if inside else "0.96"
                dot_r = 3.7 if primary else 2.6
                dots.append(
                    f"<circle cx='{sx(rg):.2f}' cy='{sy(lvl):.2f}' r='{dot_r:.2f}' fill='{color}' opacity='{opacity}'>"
        ...
        for m_lvl, m_rg in mins:
            ...
            rects.append(
                ...
                "fill='rgba(148,163,184,0.32)' stroke='rgba(226,232,240,0.42)' stroke-width='1'>"
        ...
        for idx, (lvl, rg) in enumerate(mins, start=1):
            min_labels.append(
                f"<text x='{sx(rg) + 8:.2f}' y='{sy(lvl) - 9:.2f}' fill='#f8fafc' font-size='{15 if primary else 12}' font-weight='700'>m{idx}=({lvl},{rg})</text>"
            )
        ...
        if principal:
            ...
            trivial = "<p class='svg-note'>Nontrivial Miller-Sturmfels staircase: the gold boundary is the minimal antichain of actual generator bidegrees; shaded orthants are generated over F2[x_level,x_radius], and the unshaded lattice points are the bounded complement basis in this display window.</p>"
        return f"""
        <article class='{card_class}'>
          ...
          <svg viewBox='0 0 {width} {height}' role='img' aria-label='C{dim} Miller-Sturmfels bivariate staircase from actual generator bidegrees'>
            ...
            <text x='{sx(1):.2f}' y='{sy(1)-14:.2f}' fill='#f8fafc' font-size='13'>white lattice: displayed quotient-basis complement</text>
          </svg>
        </article>
        """
