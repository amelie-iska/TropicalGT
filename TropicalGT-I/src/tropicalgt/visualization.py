...
def _write_two_parameter_bifiltration_staircase_html(
    ...
    *,
    ...
    def _svg_chain_staircase(dim: int, items: Sequence[tuple[int, int, int, str]]) -> str:
        items = list(items)
        if not items:
            return ""
        ...
        for lvl in range(0, int(y_axis_max) + 1):
            for rg in range(0, int(x_axis_max) + 1):
                ...
                color = "#1f2937" if inside else "#e8f2ff"
                opacity = "0.48" if inside else "0.92"
                dots.append(
                    f"<circle cx='{sx(rg):.2f}' cy='{sy(lvl):.2f}' r='3.4' fill='{color}' opacity='{opacity}'>"
                    f"<title>C{dim} lattice point x_radius={rg}, x_level={lvl}; {'inside generator upset' if inside else 'basis complement of generator upset'}</title></circle>"
                )
        ...
        for m_lvl, m_rg in mins:
            ...
            rects.append(
                ...
                "fill='rgba(148,163,184,0.24)' stroke='rgba(226,232,240,0.34)' stroke-width='1'>"
                f"<title>upward orthant generated by x_level^{m_lvl} x_radius^{m_rg}</title></rect>"
            )
        ...
        for lvl, rg, count, ex in sorted(items, key=lambda t: (t[0], t[1])):
            gen_marks.append(
                f"<circle cx='{sx(rg):.2f}' cy='{sy(lvl):.2f}' r='{min(13.0, 6.0 + math.sqrt(max(1, count))):.2f}' "
                f"fill='{dim_color}' stroke='#f8fafc' stroke-width='1.4'>"
                f"<title>C{dim} generator bidegree x_level={lvl}, x_radius={rg}; multiplicity={count}; example={html.escape(str(ex))}</title></circle>"
            )

        min_labels = []
        for idx, (lvl, rg) in enumerate(mins, start=1):
            min_labels.append(
                f"<text x='{sx(rg) + 8:.2f}' y='{sy(lvl) - 8:.2f}' fill='#f8fafc' font-size='13'>m{idx}=({lvl},{rg})</text>"
            )

        x_ticks = []
        stride = max(1, int(math.ceil(x_axis_max / 8)))
        ...
        trivial = ""
        if len(mins) == 1 and mins[0] in ((0, 0), (1, 0)):
            trivial = "<p class='svg-note'>This chain layer has a single minimal bidegree, so its generator upset is necessarily a simple shifted quadrant.</p>"
        else:
            trivial = "<p class='svg-note'>The stair-step boundary is the minimal antichain of actual generator bidegrees; shaded orthants are generated over F2[x_level,x_radius].</p>"
        return f"""
        <article class='stair-card'>
          <h3>C{dim} free chain module support over F2[x_level,x_radius]</h3>
          {trivial}
          <svg viewBox='0 0 {width} {height}' role='img' aria-label='C{dim} Miller-Sturmfels bivariate staircase from actual generator bidegrees'>
            <rect x='0' y='0' width='{width}' height='{height}' rx='14' fill='#050914' stroke='rgba(94,234,212,0.20)'/>
            {''.join(rects)}
            {''.join(dots)}
            <line x1='{sx(0):.2f}' y1='{sy(0):.2f}' x2='{sx(x_axis_max):.2f}' y2='{sy(0):.2f}' stroke='#e2e8f0' stroke-width='1.4'/>
            <line x1='{sx(0):.2f}' y1='{sy(0):.2f}' x2='{sx(0):.2f}' y2='{sy(y_axis_max):.2f}' stroke='#e2e8f0' stroke-width='1.4'/>
            ...
            <text x='{width-216}' y='{top+26}' fill='#e8f2ff' font-size='16'>minimal antichain: {html.escape(str(mins))}</text>
          </svg>
        </article>
        """

    staircase_svgs = "".join(_svg_chain_staircase(dim, items) for dim, items in sorted(generator_points_by_dim.items()))
...
.stair-card {{ border:1px solid rgba(94,234,212,.24); background:rgba(3,7,18,.76); border-radius:12px; padding:14px; overflow:auto; }}
...
<section class='panel'><h2>F2[x_level,x_radius] module support and homology fiber ranks</h2>{chart1}</section>
