...
def evaluate_model(model: TropicalGTModel, dataset, tokenizer: TokenGTTokenizer, seq_len: int, batch_size: int, device: torch.device) -> dict[str, float]:
    ...
    return {"nll": nll, "ppl": math.exp(min(nll, 20)), "bpb_proxy": nll / math.log(2.0), "batches": batches, "tokens": total_tokens}
